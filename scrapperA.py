...
def extract_key_specs(soup):
    ...
    text_to_embed = f"{base_json['title']} {j_type}: {text_orig.strip()}"
   
    key_specs_json['seq'] = -1
    key_specs_json['text_orig'] = text_orig.strip()
    key_specs_json['text_to_embed'] = text_to_embed

    global max_len
    if len(text_to_embed) > max_len:
        max_len = len(text_to_embed) 
...
def extract_overview(soup):
    ...
    text_to_embed = f"{overview_json['title']} {j_type}: {overview_text.strip()}"

    overview_json['seq'] = -1
    overview_json['text_orig'] = overview_text.strip()
    overview_json['text_to_embed'] = text_to_embed
    global max_len
    if len(text_to_embed) > max_len:
        max_len = len(text_to_embed) 
        name_len["url"] = base_json['url']
...
def extract_benefits(soup):
    ...
    for seq, section in enumerate(benefit_sections):
        ...
        text_to_embed = f"{benefit_json['title']} {j_type}: {text_orig}"
        
        benefit_json['seq'] = seq
        benefit_json['text_orig'] = text_orig.strip()
        benefit_json['text_to_embed'] = text_to_embed
        global max_len
        if len(text_to_embed) > max_len:
            max_len = len(text_to_embed) 
            name_len["url"] = base_json['url']
...
def extract_features(soup):
    ...
    for section in feature_sections:
        ...
        for control, content in zip(controls, contents):
            ...
            text_to_embed = f"{feature_json['title']} {j_type}: {text_orig}".strip()
            
            feature_json['seq'] = seq
            feature_json['text_orig'] = text_orig.strip()
            feature_json['text_to_embed'] = text_to_embed
            global max_len
            if len(text_to_embed) > max_len:
                max_len = len(text_to_embed) 
                name_len["url"] = base_json['url']
...
def extract_individual_specs(soup):
    ...
    for header in headers:
        ...
        if section_content:
            ...
            text_to_embed = f"{base_json['title']} {j_type}: {text_orig}"
            
            spec_json['seq'] = seq
            spec_json['text_orig'] = text_orig.strip()
            spec_json['text_to_embed'] = text_to_embed
            global max_len
            if len(text_to_embed) > max_len:
                max_len = len(text_to_embed) 
                name_len["url"] = base_json['url']
    ...
    for element in elements:
        if element.name == 'h4':
            if current_h4_title and temp_body_content:
                ...
                text_to_embed = f"{equipment_json['title']} {j_type}: {text_orig}"
...
def extract_technologies_and_services(soup):
    ...
    for element in tech_service_elements:
        ...
        text_to_embed = f"{technology_service_json['title']} {j_type}: {description}"
...
def extract_related_products(soup):
    ...
    for element in related_product_elements:
        ...
        if products:
            ...
            text_to_embed = f"{related_product_json['title']} {j_type}: {text_orig}"
...
def save_json(json_info,save_path):
    for json_data in json_info:
        if json_data:
            filepath = save_path+"/" + json_data['id'].split("!")[1] + ".json"
